...
void average_frag(process p_arr[])
{
    ...
    for (i = 0; i < MAX_BLOCKS; i++)            /* gets number of processes allocated (in a horrible way) */
    {
    ...
    for (i = 0; i < MAX_BLOCKS; i++)        /* gets total number of internal fragmentation in KB */
        if (memory[i] == FRAG)
            total_int_frag++;
    ...
    tmp_block_size = 0;
    for (i = 0; i < MAX_BLOCKS; i++)        /* gets largest free block size */
    {
        if (memory[i] == EMPTY)
            tmp_block_size++;
        if (memory[i] != EMPTY)
    ...
    for (i = 0; i < MAX_BLOCKS; i++)        /* gets total number of external fragmented blocks in KB */
        if (memory[i] == EMPTY)
            total_free_blocks++;
